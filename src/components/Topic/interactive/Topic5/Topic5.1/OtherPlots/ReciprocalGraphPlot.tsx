// src/Components/GraphViewers/ReciprocalGraphPlot.tsx
import React from 'react';

interface ReciprocalGraphPlotProps {
  width?: number;
  height?: number;
  unitSize?: number;
  k?: number; // Coefficient in y = k/x
  minX?: number;
  maxX?: number;
  minY?: number;
  maxY?: number;
...
const ReciprocalGraphPlot: React.FC<ReciprocalGraphPlotProps> = ({
  width = 300,
  height = 300,
  unitSize = 30,
  k = 1,
  ...
  // Generate points for y = k/x, avoiding x=0
  const generatePoints = () => {
    const points: [number, number][] = [];
    const step = 0.1;
    
    // Left branch (x < 0)
    for (let x = minX; x < -step; x += step) {
      if (Math.abs(x) > 0.01) { // Avoid division by zero
        const y = k / x;
        if (y >= minY && y <= maxY) {
  ...
  const generateCurvePath = () => {
    ...
    // Separate points into left and right branches
    const leftBranch = points.filter(p => p[0] < 0).sort((a, b) => a[0] - b[0]);
    const rightBranch = points.filter(p => p[0] > 0).sort((a, b) => a[0] - b[0]);
    
    let path = '';
    if (leftBranch.length > 0) {
      const leftParts = leftBranch.map((point, index) => {
    ...
    if (rightBranch.length > 0) {
      const rightParts = rightBranch.map((point, index) => {
        const { x, y } = cartesianToSvg(point[0], point[1]);
        return `${index === 0 ? 'M' : 'L'} ${x} ${y}`;
      });
  ...
  return (
    <div className="overflow-hidden max-w-full bg-white rounded-lg shadow-md">
      <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet">
        ...
        <path d={generateCurvePath()} fill="none" className="stroke-green-500 stroke-[2.5] stroke-linecap-round stroke-linejoin-round" />
        <text x={20} y={20} className="text-sm font-bold fill-green-600">{generateEquationLabel()}</text>
      </svg>
    </div>
  );
};

export default ReciprocalGraphPlot;