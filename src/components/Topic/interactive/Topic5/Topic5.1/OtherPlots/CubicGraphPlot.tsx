// src/Components/GraphViewers/CubicGraphPlot.tsx
import React from 'react';

interface CubicGraphPlotProps {
  width?: number;
  height?: number;
  unitSize?: number;
...
const CubicGraphPlot: React.FC<CubicGraphPlotProps> = ({
  width = 300,
  height = 300,
  unitSize = 30,
  a = 1,
  ...
  // Generate points based on cubic equation y = ax³ + bx² + cx + d
  const generatePoints = () => {
    const points: [number, number][] = [];
    const [startX, endX] = xRange;
    const step = 0.1; // Fine resolution for smooth curves
  ...
  const generateEquationLabel = () => {
    // Simplified label for common cases
    if (a === 1 && b === 0 && c === -3 && d === 0) return 'y = x³ - 3x';
    if (a === 1 && b === 0 && c === 0 && d === 0) return 'y = x³';
    if (a === -1 && b === 0 && c === 0 && d === 0) return 'y = -x³';
    return `y = ${a}x³ + ${b}x² + ${c}x + ${d}`.replace(/\+ -/g, '- ');
  };
  ...
  return (
    <div className="overflow-hidden max-w-full bg-white rounded-lg shadow-md">
      <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet">
        ...
        <path d={generateCurvePath()} fill="none" className="stroke-blue-500 stroke-[2.5] stroke-linecap-round stroke-linejoin-round" />
        <text x={20} y={20} className="text-sm font-bold fill-blue-600">{generateEquationLabel()}</text>
      </svg>
    </div>
  );
};

export default CubicGraphPlot;