...
interface MotionQuizProps {
  graphType: 'distance' | 'velocity' | 'displacement';
  graphExamples: GraphExample[];
  yAxisLabel: string;
  motionTypes: MotionType[];
  boundingBox: [number, number, number, number];
...
export default function GenericMotionQuiz({
  ...
  const createBoard = (graphData: GraphExample) => {
    ...
    try {
      ...
      // Add zero reference line if needed
      if (showZeroReference) {
        board.create('segment', [[boundingBox[0], 0], [boundingBox[2], 0]], {
          strokeColor: '#7f8c8d',
          strokeWidth: 1,
          dash: 2
        });
      ...
      // Mark key points for displacement graphs
  ...
  const getGraphTypeName = () => {
    switch(graphType) {
      ...
      case 'displacement': return 'Displacement-Time';
      default: return '';
    }
  };
