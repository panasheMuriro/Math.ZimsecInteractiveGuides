...
const SingularMatrixChecker: React.FC = () => {
  ...
  const findUnknownForSingular = () => {
    ...
    if (a === 'k' && typeof b === 'number' && typeof c === 'number' && typeof d === 'number') {
      // det = k*d - b*c = 0 => k = b*c / d
      if (d !== 0) {
        setUnknownValue((b * c) / d);
      } else {
        setUnknownValue(null);
      }
    } else if (b === 'k' && typeof a === 'number' && typeof c === 'number' && typeof d === 'number') {
    ...
    } else if (c === 'k' && typeof a === 'number' && typeof b === 'number' && typeof d === 'number') {
      // det = a*d - b*k = 0 => k = a*d / b
      if (b !== 0) {
        setUnknownValue((a * d) / b);
      } else {
        setUnknownValue(null);
      }
    } else if (d === 'k' && typeof a === 'number' && typeof b === 'number' && typeof c === 'number') {
      // det = a*k - b*c = 0 => k = b*c / a
      if (a !== 0) {
        setUnknownValue((b * c) / a);
      } else {
        setUnknownValue(null);
      }
    } else {
  ...
  const getUnknownLatex = () => {
    ...
    const [c, d] = findKMatrix[1];
    if (a === 'k') {
      return `${a} \\cdot ${d} - ${b} \\cdot ${c} = 0 \\implies k = \\frac{${b} \\cdot ${c}}{${d}} = ${unknownValue}`;
    } else if (b === 'k') {
      return `${a} \\cdot ${d} - k \\cdot ${c} = 0 \\implies k = \\frac{${a} \\cdot ${d}}{${c}} = ${unknownValue}`;
    } else if (c === 'k') {
      return `${a} \\cdot ${d} - ${b} \\cdot k = 0 \\implies k = \\frac{${a} \\cdot ${d}}{${b}} = ${unknownValue}`;
    } else if (d === 'k') {
